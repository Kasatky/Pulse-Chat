...
uploadRouter.post('/MsgImg', async (req, res) => {
	const { user } = res.locals;
	...
	const newFileName = encodeURI(Date.now()+'-'+file.name)
	const fileDirPath = `${__dirname}../../public/msgImg/${newFileName}`
	const image = await ProfilePic.findOne({where: {id:user.id}})
	if (image) {
		image.fileName = newFileName
		image.direction = fileDirPath
		image.save()
	} else await ProfilePic.create({
		id: user.id,
		fileName: newFileName,
